...
import type { AplusCore2Lesson, AplusCore2Section, AplusMultipleChoiceQuestion, AplusResource } from '../../data/aplusCore2';
import { modules } from '../../data/modules';
import { requestAiCoachFeedback } from '../../lib/ai/coachClient';
import type { AiCoachFeedback } from '../../lib/ai/coachTypes';
import {
  addPdEntry,
...
export type CertificationPathwayClientProps = {
  certificationId: string;
  certificationTitle: string;
  pageTitle: string;
  pageDescription: string;
  sections: AplusCore2Section[];
  lessons: AplusCore2Lesson[];
  coreResources: AplusResource[];
  stats: {
    lessonCount: number;
    sectionCount: number;
...
export default function CertificationPathwayClient({
  certificationId,
  certificationTitle,
  pageTitle,
  pageDescription,
  ...
  coreResources,
  stats
}: CertificationPathwayClientProps) {
  ...
  const latestAttemptByLesson = useMemo(() => {
    ...
    return latest;
  }, [progress.certificationAssessmentAttempts, certificationId]);

  const selectedLesson = lessons.find((lesson) => lesson.id === selectedLessonId) || lessons[0];
  const filteredLessons =
    selectedSectionId === 'all' ? lessons : lessons.filter((lesson) => lesson.sectionId === selectedSectionId);
  ...
  if (!selectedLesson) {
    return <div className="p-8 text-slate-600">No lessons configured for this pathway yet.</div>;
  }
  ...
  async function gradeAnswer() {
    ...
    const result = await requestAiCoachFeedback({
      ...
      extraContext: `${certificationTitle}. Objective ${selectedLesson.objectiveId}: ${selectedLesson.objectiveTitle}. Lesson: ${selectedLesson.title}. DCS application: ${selectedLesson.dcsApplication}. Multiple-choice score: ${multipleChoiceScore}/100 (${multipleChoiceCorrectCount}/${selectedLesson.assessment.multipleChoice.length}).`
    });
    ...
    const pdEntryId = `${certificationId}-${selectedLesson.id}-pd-${Date.now()}`;
    const attemptId = `${certificationId}-${selectedLesson.id}-attempt-${Date.now()}`;
    ...
    const pdEntry: PdEntry = {
      ...
      createdAtIso: nowIso,
      type: 'ai-coaching',
      title: `${certificationTitle} assessment: ${selectedLesson.title}`,
      minutes: 15,
    ...
    <div className="space-y-6">
      <section className="rounded-[2rem] border border-slate-200 bg-white p-6 shadow-sm">
        <div className="flex flex-col gap-5 xl:flex-row xl:items-end xl:justify-between">
          <div className="max-w-4xl">
            ...
            <h1 className="mt-3 text-4xl font-semibold tracking-tight text-slate-900">{pageTitle}</h1>
            <p className="mt-3 text-sm leading-7 text-slate-600">{pageDescription}</p>
          </div>
          <div className="rounded-3xl bg-slate-100 px-5 py-4 text-sm text-slate-700">
            {stats.lessonCount} lessons | {stats.totalRunTime} | {stats.examCode}
          </div>
        </div>
      </section>
      ...
      <section className="grid gap-4 md:grid-cols-4">
        {[
          ...
          ['Assessed', `${completedLessons}/${lessons.length}`],
          ['Average', averageScore === null ? 'No score' : `${Math.round(averageScore)}/100`],
          ['Domains', stats.objectiveCount]
        ].map(([label, value]) => (
        ...
        <div className="space-y-6">
          ...
          <section className="rounded-[2rem] border border-slate-200 bg-white p-6 shadow-sm">
            ...
            <div className="mt-5 rounded-2xl border border-slate-200 bg-slate-50 p-4">
              <div className="flex flex-col gap-2 md:flex-row md:items-center md:justify-between">
                ...
                <div className="rounded-full bg-white px-3 py-1 text-xs font-semibold text-slate-900">
                  {multipleChoiceCorrectCount}/{multipleChoiceQuestionCount} correct
                </div>
              </div>
            ...
            <textarea
              ...
              placeholder="Use fake or anonymised examples only."
            />
